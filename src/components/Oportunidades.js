// src/components/Oportunidades.js - VERSÃO AUDITADA E CORRIGIDA
...
const Oportunidades = () => {
  ...
  const handleBuscaManual = async () => {
    ...
    // O objeto de parâmetros agora envia EXATAMENTE o que a função 'buscar_oportunidades' espera.
    const params = {
  ...
  const handlePertoDeCasa = async () => {
    ...
    // Esta chamada já estava correta, enviando os parâmetros que a função 'buscar_oportunidades_agrupadas' espera.
  ...
  if (isLoading) return <div className="oportunidades-container"><p>Carregando...</p></div>;

  return (
    <div className="oportunidades-container">
      <header className="oportunidades-header">
        <button onClick={() => navigate('/dashboard')} className="btn-voltar">← Início</button>
        <h1>Oportunidades</h1>
      </header>
      ...
      <section className="secao-busca">
        ...
        <div className="filtros-container">
          <div className="filtro-item">
            ...
            <select id="filtro-habilidade" value={filtroHabilidade} onChange={(e) => setFiltroHabilidade(e.target.value)} disabled={modoBusca === 'perto_de_casa'}>
          ...
          <div className="filtro-item">
            ...
            <input id="filtro-data" type="date" value={filtroData} onChange={(e) => setFiltroData(e.target.value)} />
          </div>
        </div>
